...
function TransactionHistory() {
  ...
  const isValidTransactions = Array.isArray(transactions);
  ...
  useEffect(() => {
    ...
    // Filter by type
    if (typeFilter !== 'All') {
      temp = temp.filter((t) => t.type && t.type.toLowerCase() === typeFilter.toLowerCase());
    }
    ...
    if (sortBy === 'Date') {
      ...
      temp.sort((a, b) => (b.amount || 0) - (a.amount || 0));
    }

    setFiltered(temp);
  }, [search, typeFilter, sortBy, transactions, isValidTransactions]);
          ...
          <div className="flex flex-col gap-4">
            {!isValidTransactions ? (
              ...
              filtered.map((item, index) => (
                <TransactionCard item={item} key={item._id || index} />
              ))
            )}
          </div>
        </div>
